...
pred_csv_path = results_dir / "tables" / "2025_04_19" / "true_vs_predicted_LGBM_19_0709.csv"
pred_orig_df = pd.read_csv(pred_csv_path, index_col=0)
# Subset to all passenger Ids that are wrongly predicted
pred_df = pred_orig_df[pred_orig_df["true_transport"]!=pred_orig_df["predicted_transport"]]
# Load the original x_val data
val_csv_path = results_dir / "intermediate" / "2025_04_19" / "x_val.csv"
...
# Add the true transport and predicted transport columns to the good and bad dataframes
good_df['true_transport'] = pred_orig_df.loc[good_df.index, 'true_transport']
bad_df['true_transport'] = pred_orig_df.loc[bad_df.index, 'true_transport']
good_df['predicted_transport'] = pred_orig_df.loc[good_df.index, 'predicted_transport']
bad_df['predicted_transport'] = pred_orig_df.loc[bad_df.index, 'predicted_transport']
