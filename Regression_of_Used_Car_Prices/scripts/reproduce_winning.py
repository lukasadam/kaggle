...
df_original = pd.read_csv(
    "/Users/adaml9/Private/kaggle/Regression_of_Used_Car_Prices/data/used_car_price_dataset/used_cars.csv"
)
df_test = pd.read_csv(
...
for feature in df_train.columns:
    feat_stat[feature] = np.round(df_train[feature].value_counts().describe())[
        ["count", "mean", "min", "max"]
    ].astype(int)
...
def feat_eng(data):
    ...
    # numerical features
    df["horsepower"] = (
        df["engine"].str.extract(r"(\d+\.?\d*)HP").astype("float").fillna(-1)
    )
    df["displacement"] = (
        df["engine"].str.extract(r"(\d+(\.\d+)?)L")[0].astype(float).fillna(-1)
    )
    return df


feat_cols = df_test.columns.to_list()
data_concat = feat_eng(
    pd.concat([df_train[feat_cols], df_original[feat_cols], df_test])
)
...
df = pd.DataFrame({"price": test_pred_ensemble}, index=test_df.index)
