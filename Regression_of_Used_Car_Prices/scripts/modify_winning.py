...
def crossvalidate(label, data, test_data, n_bags=5, n_splits=5):
    ...
    for fold, (train_index, valid_index) in enumerate(cv.split(data, y.astype(str))):
        ...
        X_train, X_valid = (
            ...
            data.drop(columns="price").iloc[valid_index],
        )
        y_train, y_valid = y.iloc[train_index], y.iloc[valid_index]

        if label == "CAT":
            X_train, X_valid, X_test_temp = target_encoding(
        ...
        for i in range(n_bags):
            ...
            if label == "NN":
                y_pred = m.predict(
                    [X_valid_cats, X_valid_nums], verbose=1, batch_size=512
                ).flatten()
                pred = m.predict(
                    [X_test_cats, X_test_nums], verbose=1, batch_size=512
                ).flatten()
            elif label == "CAT":
    ...
    if label == "CAT":
        print(
            f"#ROC-AUC score: {np.array(scores)}, mean: {np.array(scores).mean():.7f} (+- {np.array(scores).std():.7f})"
        )
        print(
            f"#Oof score: {roc_auc_score(y, oof):.7f} ({int(np.round(elapsed_time.total_seconds() / 60))} min)"
        )
    else:
        print(
            f"#RMSE: {np.array(scores)}, mean: {np.array(scores).mean():.7f} (+- {np.array(scores).std():.7f})"
        )
        print(
            f"#Oof score: {np.sqrt(mean_squared_error(y, oof)):.7f} ({int(np.round(elapsed_time.total_seconds() / 60))} min)"
        )
    if label in ["Ridge"]:
...
def target_encoding(
    X_train, X_test, df_test, cat_cols, new_cols=["median"], smoothing_param=0.0001
):
    """
    ...
    def add_new_feature(name, feature):
        agg = X_train.groupby(feature)["price"].agg(["count", "mean", "median", "std"])
        counts = agg["count"]
        if name == "mean":
    ...
    for feature in cat_cols:
        for name in new_cols:
            ...
            X_train_target[feature_name] = (
                X_train_target[feature]
                .map(price_per_category)
                .astype(float)
                .fillna(target_value)
                .astype(int)
            )
            X_test_target[feature_name] = (
                (X_test_target[feature].map(price_per_category))
                .astype(float)
                .fillna(target_value)
                .astype(int)
            )
            df_test_target[feature_name] = (
                (df_test_target[feature].map(price_per_category))
                .astype(float)
                .fillna(target_value)
                .astype(int)
            )
...
hyperparameters = {
    "CAT": {
        "iterations": 10000,  # instead of early_stopping_rounds
...
predictor = TabularPredictor(label="price", eval_metric="accuracy").fit(
    train_data=data_train2,
    hyperparameters=hyperparameters,
    holdout_frac=0.1,  # similar to early stopping
)

oof_cat2 = predictor.predict_proba(data_train2)[1].values
test_pred_cat2 = predictor.predict_proba(data_test)[1].values
predictor.leaderboard(data_train2, silent=True)


from custom_models import SVRModel
...
oof_svr = predictor.predict(data_train).values
test_pred_svr = predictor.predict(data_test).values
...
oof_lgb1 = predictor.predict(data_train).values
test_pred_lgb1 = predictor.predict(data_test).values
predictor.leaderboard(data_train, silent=True)
...
oof_lgb1_st = predictor.predict(data_train2).values
test_pred_lgb1_st = predictor.predict(data_test2).values
predictor.leaderboard(data_train2, silent=True)

data_concat, CAT_SIZE, CAT_EMB = label_encoding(pd.concat([data_train2, data_test2]), 5)
data_train3 = data_concat.loc[data_train2.index]
# Merge price column back to the training data
data_train3["price"] = data_train2["price"]
data_test3 = data_concat.loc[data_test2.index]
...
predictor = TabularPredictor(label="price", eval_metric="rmse").fit(
    train_data=data_train3,
    hyperparameters=hyperparameters,
    holdout_frac=0.1,  # similar to early stopping
)
oof_lgb5_st = predictor.predict(data_train3).values
test_pred_lgb5_st = predictor.predict(data_test3).values
predictor.leaderboard(data_train3, silent=True)
...
data_train2 = data_concat.loc[data_train.index]
# Merge price column back to the training data
data_train2["price"] = data_train["price"]
data_test2 = data_concat.loc[data_test.index]


def one_hot_encoding(df_train, df_test, cat_cols, cat_cols_old):

    encoder = OneHotEncoder(
        sparse_output=False, dtype=np.int32, handle_unknown="ignore"
    )
...
feat_cols = data_test.columns.to_list()
data_train2, data_test2, cat_cols = one_hot_encoding(
    data_train2, data_test2, cat_cols2, cat_cols
)

feat_cols = data_test2.columns.to_list()
data_concat, CAT_SIZE, CAT_EMB = label_encoding(
    pd.concat([data_train2[feat_cols], data_test2]), 20
)
...
tf.keras.utils.plot_model(
    model=build_model(data_train2), show_shapes=True, rankdir="TB"
)

data_train2 = data_train2.drop(columns=["engine_hp", "displacement"])
data_test2 = data_test2.drop(columns=["engine_hp", "displacement"])
num_cols = ["milage"]
test_pred_nn, oof_nn, score_nn = crossvalidate(
    "NN", data_train2, data_test2, n_bags, n_folds
)
